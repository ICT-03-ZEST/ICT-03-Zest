	...
	<div class="btnNum" align="center">
        <div>
			...
			<c:if test="${paging.startPage > 3}"> <!-- 시작페이지가 4부터 시작할때 이전버튼이 보임-->
				<a class="prev" href="${path}/board.bc?pageNum=${paging.prev}&board_category=공연후기" class="prev"> << </a>
			</c:if>
        
        	<!-- 페이지 번호 처리 -->
			<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
				<a href="${path}/board.bc?pageNum=${num}&board_category=공연후기">${num}</a>
			</c:forEach>
        
        	<!-- 다음 버튼 활성화 -->	
			<c:if test="${paging.endPage < paging.pageCount}"> <!-- 마지막페이지가 전체페이지수()보다 작을때 다음버튼이 보임-->
				<a class="next" href="${path}/board.bc?pageNum=${paging.next}&board_category=공연후기" class="next"> >> </a>
			</c:if>
        </div>
    </div>

...
@WebServlet("*.bc")
@MultipartConfig(location="C:\\dev\\workspace\\js_pj_fasticat\\src\\main\\webapp\\resources\\upload", 
			fileSizeThreshold=1024*1024, maxFileSize=1024*1024*5, maxRequestSize=1024*1024*5*5)
public class BoardController extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final String IMG_UPLOAD_DIR = "C:\\dev\\workspace\\js_pj_fasticat\\src\\main\\webapp\\resources\\upload";
	...
	public void action(HttpServletRequest request, HttpServletResponse response) 
		...
		// 1-1. 게시판 상세페이지 
			...
			System.out.println("컨트롤러 category: " + category);
		...
		//2-1. 게시글 추가 페이지
			...
			viewPage = "customer/mypage/myWriting.jsp";
		}
		...
		// 2-2. 게시글 추가 처리
			...
			viewPage = "customer/mypage/boardInsertAction.jsp";
		}
		
		// 3-1.게시글 수정 페이지 - 1. 게시판에서 본인게시글이면 수정가능 2. 마이페이지에서 수정가능
		else if(url.equals("/myBoardUpdate.bc")) {
			System.out.println("컨트롤러 - myBoardUpdate.bc");
			
			serv.boardDetailAction(request, response);
			
			viewPage = "customer/mypage/myBoardUpdate.jsp";
		}
		...
		// 4.게시글 삭제 -  1. 게시판에서 본인게시글이면 삭제가능 2. 마이페이지에서 삭제가능
			...
			viewPage = "customer/mypage/boardDeleteAction.jsp";
		}
		
		else if(url.equals("/logout.bc")) {

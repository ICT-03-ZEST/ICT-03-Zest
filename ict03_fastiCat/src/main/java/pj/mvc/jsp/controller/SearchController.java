...
public class SearchController extends HttpServlet {
	...
	public void action(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 공연목록 조회
		else if (url.equals("/searchEvent.sc")) {
			System.out.println("<<< url ==> /searchEvent.cc>>>");

			service.concertListAction(request, response);
			viewPage = "customer/search/searchEvent.jsp";
		}
		// 공연목록 조회 결과
		else if (url.equals("/searchconcertList.sc")) {
			System.out.println("<<< url ==> /searchconcertList.cc>>>");

			service.concertListAction(request, response);
			viewPage = "customer/search/searchconcert.jsp";
		}

		// [ 검색창 세부검색 ]
		else if (url.equals("/search_detailList.sc")) {
